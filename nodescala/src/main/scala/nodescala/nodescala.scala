...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    response.toStream.takeWhile(s => !token.isCancelled).
      foreach(part => exchange.write(part))
    exchange.close()
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    Subscription(listener.start(),
    Future.run() {
      ct => async {
        while (ct.nonCancelled) {
          val (request, response) = await(listener.nextRequest())
          async {
            respond(response, ct, handler(request))
          }
        }
      }
    })
